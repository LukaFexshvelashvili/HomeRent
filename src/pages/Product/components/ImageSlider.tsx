...
export default function ImageSlider() {
  ...
  return (
    <div className="flex-[1.3] bg-whiteMain rounded-block relative flex items-center justify-center  shadow-sectionShadow">
      ...
      <div className="flex items-center gap-4 absolute bottom-3 z-[3]">
        {imageList.map((item: string, i: number) => (
          <div
            key={i}
            onClick={() => setSlider(i)}
            className={`h-[40px] aspect-video rounded-[5px] outline outline-[2px] shadow-[0px_0px_20px_rgba(0,_0,_0,_0.35)] overflow-hidden cursor-pointer ${
      ...
      <div className="relative overflow-hidden h-full w-full rounded-block z-[1]">
        <img
      ...
      <button
        onClick={() => sliderPrev()}
        className="h-16 aspect-square rounded-circle bg-whiteMain p-5 absolute z-[5] left-0 -translate-x-1/4 shadow-slideNavShadow"
      >
        <LeftArrowIcon className="h-full aspect-square" />{" "}
      </button>{" "}
      <button
        onClick={() => sliderNext()}
        className="h-16 aspect-square rounded-circle bg-whiteMain p-5 absolute z-[5] right-0 translate-x-1/4 shadow-slideNavShadow"
      >
